...
MixedModelsBLMM   <- function(jaspResults, dataset, options, state = NULL){
  ...
  # fit the model
  if(is.null(jaspResults[["mmModel"]]) & .mmReady(options)).mmFitModelB(jaspResults, dataset, options)
  ...
  if(!is.null(jaspResults[["mmModel"]])){
    ...
    if(length(options$marginalMeans) > 0 & options$marginalMeansContrast & !is.null(jaspResults[["EMMresults"]])).mmContrasts(jaspResults, options, "BLMM")
    
    
    # trends
    if(length(options$trendsTrend) > 0 & length(options$trendsVariables) > 0).mmTrends(jaspResults, dataset, options, "BLMM")
    if(options$trendsContrast & length(options$trendsTrend) > 0 & length(options$trendsVariables) > 0 & !is.null(jaspResults[["EMTresults"]])).mmContrasts(jaspResults, options, "BLMM", what = "Trends")
  }
