...
#'
#' Bayesian Generalized Linear Mixed Models allow you to model a linear relationship between one or more explanatory variable(s) and a dependent variable in cases where the observations are not independent, but clustered within one or several random effects grouping factors (e.g., repeated measures across participants or items, children within schools). They are generalizations of Bayesian Linear Mixed Models and allow you to model response variables that are not continuous using different likelihoods and link functions.
#' ## Assumptions
...
#'    Defaults to \code{FALSE}.
#' @param marginalMeansResponse, Whether the estimated marginal means should be computed on the response scale or untransformed linear scale. The response scale is selected by default.
#'    Defaults to \code{TRUE}.
#' @param marginalMeansSd, Specifies the 'levels' of continuous variables (expressed in standard deviations) for which the estimated marginal means are computed.
...
MixedModelsBGLMM <- function(
   ...
   return(jaspBase::runWrappedAnalysis("jaspMixedModels", "MixedModelsBGLMM", "MixedModelsBGLMM.qml", options, version, TRUE))
}