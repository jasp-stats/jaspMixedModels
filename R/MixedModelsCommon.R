...
.mmModelFormula  <- function(options, dataset) {
  ...
  for (tempRe in options[["randomEffects"]]) {
    ...
    tempVars     <-
      sapply(tempVars, function(x)
        paste(unlist(x), collapse = "*"))
    tempVarsRem <- tempVarsRem[!is.na(tempVarsRem)]
    tempVarsRem <-
      sapply(tempVarsRem, function(x)
        paste(unlist(x), collapse = "*"))
    ...
    if (!is.null(meToRemove)) {
      tempVars <-
        tempVars[!tempVars %in% unique(as.vector(sapply(meToRemove, function(x)
    ...
    if (!is.null(teToRemove)) {
      teToRemove <-
        unique(as.vector(sapply(teToRemove, function(x)
          tempVars[grepl(x, tempVars, fixed = TRUE)])))
...
.mmSimplifyTerms <- function(terms) {
  if (length(terms) > 1) {
    ...
    splitTerms  <-
      sapply(splitTerms, function(x)
        trimws(x, which = c("both")))

    termsToRemove <- rep(NA, length(splitTerms))
    for (i in 1:length(terms)) {
      termsToRemove[i] <-
        any(sapply(splitTerms[-i], function(x)
          all(splitTerms[[i]] %in% x)))
    }
...
.mmAddedRETerms  <- function(terms, removed) {
  ...
  if (length(terms) > 1 && length(removed) >= 1) {
    ...
    splitRemoved <-
      sapply(splitRemoved, function(x)
        trimws(x, which = c("both")))
...
.mmFitModel      <- function(jaspResults, dataset, options, type = "LMM") {
  ...
  if (type == "LMM") {
    dependencies <- c(.mmDependenciesLMM, seedDependencies)
  } else if (type == "GLMM") {
    dependencies <- c(.mmDependenciesGLMM, seedDependencies)
  }
...
.mmSummaryAnova  <- function(jaspResults, dataset, options, type = "LMM") {
  ...
  for (i in 1:nrow(model$anova_table)) {
    ...
    tempRow <- list(effect = effectName,
    ...
    if (options$pvalVS) {
      ...
      if (options$method == "PB") {
        tempRow$pvalBootVS <-
          VovkSellkeMPR(tempRow$pvalBoot)
      }
    }
...
.mmSummaryRE     <- function(jaspResults, options, type = "LMM") {
  ...
  if (is.list(model$full_model)) {
    ...
    tempRow <- list(std      = sigma(model$full_model),
...
.mmSummaryFE     <- function(jaspResults, options, type = "LMM") {
  ...
  if (ncol(FEcoef) >= 4) FEsummary$addColumnInfo(name = "pval",       title = gettext("p"),          type = "pvalue")
...
.mmPlot          <- function(jaspResults, dataset, options, type = "LMM") {
  ...
  if (options$plotsEstimatesTable) {
    ...
    for (i in 1:nrow(plotData)) {
      tempRow <- list()
      for (v in attr(plotData, "pri.vars")) {
        tempRow[v] <- as.character(plotData[i, v])
      }
...
.mmMarginalMeans <- function(jaspResults, dataset, options, type = "LMM") {
  ...
  if (type %in% c("LMM", "GLMM")) {
    ...
    if (options$marginalMeansCompare) {
      EMMsummary$addColumnInfo(
        name = "stat",
        title = ifelse(colnames(emmTest)[ncol(emmTest) - 1] == "t.ratio", gettext("t"), gettext("z")),
        type = "number"
      )
  ...
  for (i in 1:nrow(emmTable)) {
    ...
    if (type %in% c("LMM", "GLMM")) {
      ...
      tempRow$estimate <-
        emmTable[i, grep("SE", colnames(emmTable)) - 1]
...
.mmTrends        <- function(jaspResults, dataset, options, type = "LMM") {
  ...
  trendsAt      <<- at
  trendsType    <<- if (type == "LMM" || (type == "GLMM" &&
                                          options$family == "gaussian" &&
                                          options$link == "identity"))
    "LMM"
  else
  ...
  if (type == "LMM") {
    dependenciesAdd <-
      c(dependenciesAdd, "trendsDf", "trendsOverride")
  }
...
.mmFitModelB      <- function(jaspResults, dataset, options, type = "BLMM") {
  ...
  if (type == "BLMM") {
    model <- tryCatch(stanova::stanova(
      ...
      control           = list(maxTreedepth = options$max_treedepth),
...
.mmSummaryFEB     <- function(jaspResults, options, type = "BLMM") {
  ...
  namesSummary <- rownames(modelSummary)
  feSummary    <-
    modelSummary[!grepl("b[", namesSummary, fixed = T) &
                   !namesSummary %in% c("mean_PPD", "log-posterior") &
                   namesSummary != "sigma" &
                   !grepl("Sigma[", namesSummary, fixed = T), ]
  ...
  for (i in 1:length(modelSummary)) {
    ...
    if (names(modelSummary)[i] == "Model summary") {
      ...
      if (options$show == "deviation") {
        tableName <-
          gettextf("%s (differences from intercept)",varName)
    ...
    if (varName != "Intercept" && nrow(tempSummary) > 1) {
      tempTable$addColumnInfo(name = "level",
                              title = gettext("Level"),
                              type = "string")
    }
    tempTable$addColumnInfo(name = "estimate",
                            title = gettext("Estimate"),
                            type = "number")
    tempTable$addColumnInfo(name = "se",
                            title = gettext("SE"),
                            type = "number")
    tempTable$addColumnInfo(
      name = "lowerCI",
      title = gettext("Lower"),
      type = "number",
      overtitle = gettextf("%s%% CI", 100 * options$summaryCI)
    ...
    tempTable$addColumnInfo(name = "rhat",
                            title = gettext("R-hat"),
                            type = "number")
    tempTable$addColumnInfo(name = "ess_bulk",
                            title = gettext("ESS (bulk)"),
                            type = "number")
    tempTable$addColumnInfo(name = "ess_tail",
                            title = gettext("ESS (tail)"),
                            type = "number")

    if (tableName == gettext("Model summary")) {
      if(options$dependentVariable != "" &&
         length(options$fixedVariables) > 0 &&
         length(options$randomVariables) == 0) {
    ...
    removedTe <- jaspResults[["mmModel"]]$object$removedTe
    addedRe   <- jaspResults[["mmModel"]]$object$addedRe
    if (length(removedMe) > 0) {
      for (j in 1:length(removedMe)) {
        tempTable$addFootnote(.mmMessageOmmitedTerms1(removedMe[[j]], names(removedMe)[j]),
                              symbol = gettext("Note:"))
      }
    }
    if (length(removedTe) > 0) {
      for (j in 1:length(removedTe)) {
        tempTable$addFootnote(.mmMessageOmmitedTerms2(removedTe[[j]], names(removedTe)[j]),
                              symbol = gettext("Note:"))
      }
    }
    if (length(addedRe) > 0) {
      for (i in 1:length(addedRe)) {
        tempTable$addFootnote(.mmMessageAddedTerms(addedRe[[i]], names(addedRe)[i]), symbol = gettext("Note:"))
      }
    }
    if (jaspResults[["n_missing"]]$object != 0) {
      tempTable$addFootnote(.mmMessageMissingRows(jaspResults[["n_missing"]]$object))
    }
    if (type == "BGLMM") {
...
.mmGetPlotSamples <- function(model, pars, options) {
  ...
  if (length(pars) == 1) {
    ...
    for (cf in 1:coefs) {
      ...
      plotData[[dimnames(samples)$Parameter[cf]]] <- list(
        samp = data.frame(
...
.rstanPlotHist  <- function(plotData) {
  ...
  base      <-
    ggplot2::ggplot(plotData$samp, ggplot2::aes_string(x = "value"))
...
.rstanPlotDens  <- function(plotData, separate_chains = TRUE) {
  ...
  graph + ggplot2::xlab(unique(plotData$samp$parameter))
...
.rstanPlotScat  <- function(plotData) {
  ...
  p1    <-
    plotData$samp$parameter == levels(plotData$samp$parameter)[1]
  p2    <-
    plotData$samp$parameter == levels(plotData$samp$parameter)[2]
...
.rstanPlotAcor  <- function(plotData, lags = 30) {
  ...
  ac_dat   <-
    rstan:::.ac_plotData(dat = plotData$samp,
                         lags = lags,
                         partial = FALSE)
