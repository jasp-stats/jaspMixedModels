...
#'
#' Generalized Linear Mixed Models allow you to model a linear relationship between one or more explanatory variable(s) and a dependent variable in cases where the observations are not independent, but clustered within one or several random effects grouping factors (e.g., repeated measures across participants or items, children within schools). They are generalizations of Linear Mixed Models and allow you to model response variables that are not continuous using different likelihoods and link functions.
#' ## Assumptions
...
MixedModelsGLMM <- function(
   ...
   return(jaspBase::runWrappedAnalysis("jaspMixedModels", "MixedModelsGLMM", "MixedModelsGLMM.qml", options, version, TRUE))
}